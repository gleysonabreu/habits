...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (session.user.username === username) {
      return NextResponse.json({ message: 'Esse nome de usuário já é seu!' }, { status: 400 });
    }
    ...
    if (findUsername) {
      return NextResponse.json({ message: 'Nome de usuário já está cadastrado em outra conta!' }, { status: 400 });
    }
