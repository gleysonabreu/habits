...
export function Settings({ open, close }: SettingsProps) {
  ...
  const handleUpadateUsername: SubmitHandler<UpdateUsernameForm> = async (
    data,
  ) => {
    try {
  ...
  async function handleEnableOrDisableProfile(checked: boolean) {
    try {
      setLoading(true);
      const url = checked
        ? '/api/v1/users/profile/enabled'
        : '/api/v1/users/profile/disabled';
      ...
      if (response.status === 204) {
        setMessageGlobal({
          ...
          text: `Seu perfil foi colocado em modo ${
            checked ? 'público' : 'oculto'
          }!`,
          type: 'success',
        });
        setIsPublic(checked);
    ...
    <SideOvers title="Configurações" open={open} onClose={onCloseSlideOver}>
      <div className="relative mt-6 flex-1 px-4 sm:px-6">
        <div className="mx-auto mb-0 mt-8 max-w-md space-y-4 border-b border-zinc-700/10 pb-6">
          {messageGlobal && <Alert {...messageGlobal} />}
          <Form.Field>
        ...
        <FormProvider {...updateUsernameForm}>
          <form
            onSubmit={handleSubmit(handleUpadateUsername)}
          ...
          >
            <Form.Field>
              ...
              <div className="flex flex-col gap-2 items-center justify-center p-3 border border-zinc-800/90 rounded-lg border-dashed select-none">
                ...
                <CopyUrl
                  url={`${process.env.NEXT_PUBLIC_URL}/me/${session?.user.username}`}
                />
              </div>
