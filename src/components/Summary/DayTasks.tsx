...
export function DayTaks({ habitId, date, handleCompleted, handleAmount }: DayTasksProps) {
  const weekDay = dayjs(date).get('day');
  const dateNow = dayjs(date).format('YYYY-MM-DD');
  const { data: dayTasksList, error, isLoading } = useSWR<DayTasksList, Error>(`/api/v1/days/${habitId}?date=${dateNow}&weekDay=${weekDay}`);
  ...
  function handleAddCompleted(taskId: string) {
    ...
    if (dayTasksList) {
      ...
      if (findTask) {
        tasksCompleted = dayTasksList.completedTasks.filter(id => id !== taskId);
      } else {
  ...
  if (error) {
    return <Alert text="Ocorreu um erro interno, tente novamente." type="danger" title="Internal Server Error" />
  }

  return (
    <div className='flex gap-3 flex-col w-full p-2 max-h-36 overflow-x-auto'>
      {dayTasksList?.possibleTasks.length === 0 ? (
        <span className='text-zinc-400 dark:text-zinc-500'>Nenhuma tarefa encrontada.</span>
      ) : dayTasksList?.possibleTasks.map(dayTask => {
        const isCompleted = dayTasksList.completedTasks.includes(dayTask.id);
        return (
          <Checkbox key={dayTask.id} date={date} handleCompleted={handleAddCompleted} taskName={dayTask.name} taskId={dayTask.id} isCompleted={isCompleted} />
        )
      })}
    </div>
  );
