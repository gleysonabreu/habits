...
export function SignIn({ error: errorType }: SignInProps) {
  const errors: Record<SignInErrorTypes, string> = {
    ...
    CredentialsSignin: 'Loguin falhou. Verifique se os dados estão corretos.',
  ...
  return (
    <div className="flex w-full h-full items-center flex-col max-w-lg bg-zinc-200 p-5 rounded-lg dark:bg-zinc-900 gap-10">
      ...
      <h1 className="font-bold text-xl text-center text-zinc-800 dark:text-zinc-100">
        Acesse a plataforma
      </h1>
      <div className="flex flex-col flex-1 w-full items-center justify-center gap-4">
        <Button
          ...
          size="xl"
        >
          <GithubLogo size={25} weight="fill" /> Github
        </Button>
        <Button
          title="Twitch"
          ...
          variant="purple"
        >
          <TwitchLogo size={25} weight="fill" /> Twitch
        </Button>
        <Button
          title="Google"
          ...
          variant="white"
        >
          <GoogleLogo size={25} weight="fill" /> Google
        </Button>
      </div>
    </div>
  );
